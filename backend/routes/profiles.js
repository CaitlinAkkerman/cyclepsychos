...
router.post('/', authMiddleware, (req, res) => {
  const { name, age, role, cycleLength, periodLength } = req.body;
  const db = req.db;
  ...
  db.get('SELECT * FROM households WHERE userId = ?', [userId], (err, household) => {
    ...
    db.run(
      ...
      function(err) {
        ...
        res.json({ id: profileId, name, age, role, cycleLength, periodLength });
      }
    );
  });
});
...
router.put('/:id', authMiddleware, (req, res) => {
  const { age, cycleLength, periodLength } = req.body;
  const db = req.db;
  const userId = req.userId;
  const profileId = req.params.id;
